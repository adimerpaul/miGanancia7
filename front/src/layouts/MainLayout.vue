    ...
    <q-drawer v-model="leftDrawerOpen" show-if-above bordered>
      <q-layout view="lHh lpr lFf" container class="">
        ...
        <q-page-container>
          ...
          <q-list >
            <q-item clickable v-ripple :class="`bg-white ${option.color}`" @click="option.option" v-for="(option, index) in options" :key="index">
              <q-item-section avatar>
...
export default defineComponent({
  ...
  setup () {
    ...
    const options = [
      { color: 'text-grey', label: 'Movimientos', link: '/', icon: 'o_account_balance_wallet' },
      { color: 'text-grey', label: 'Negocios', link: '/negocios', icon: 'o_store' },
      { color: 'text-grey', label: 'Perfil', link: '/perfil', icon: 'o_person' }
    ]
    return { leftDrawerOpen, globalStore, q, links, shopDialog, shopCreate, shop, loading, options }
  },
  methods: {
    negocioDelete () {
      this.q.dialog({
